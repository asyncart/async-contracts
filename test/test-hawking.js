...
contract("AsyncArtwork", function(accounts) {
	...
	it ("mints Hawking by artist A", function() {
		...
		return artworkInstance.mintArtwork(expectedArtworkTokenId, artworkURI, controlTokenArtists).then(function(tx) {
			return artworkInstance.expectedTokenSupply().then(function(expectedTokenSupply) {
				// should be total layers count plus 1 for the base
				assert(expectedTokenSupply == artistLayers.length + 1, "Wrong token count")
				...
				return artworkInstance.whitelistedCreators(ARTIST_A).then(function(isWhitelisted) {
					...
					return artworkInstance.whitelistedCreators(COLLECTOR_A).then(function(isWhitelisted) {
						...
						console.log("ControlToken = " + controlTokenIds[controlTokenIndex]);return artworkInstance.setupControlToken(expectedArtworkTokenId, controlTokenIds[controlTokenIndex], controlTokenURIs[controlTokenIndex], minValues[controlTokenIndex], maxValues[controlTokenIndex], startValues[controlTokenIndex],{from:ARTIST_A}).then(function(tx) {controlTokenIndex++;console.log("ControlToken = " + controlTokenIds[controlTokenIndex]);return artworkInstance.setupControlToken(expectedArtworkTokenId, controlTokenIds[controlTokenIndex], controlTokenURIs[controlTokenIndex], minValues[controlTokenIndex], maxValues[controlTokenIndex], startValues[controlTokenIndex],{from:ARTIST_B}).then(function(tx) {controlTokenIndex++;console.log("ControlToken = " + controlTokenIds[controlTokenIndex]);return artworkInstance.setupControlToken(expectedArtworkTokenId, controlTokenIds[controlTokenIndex], controlTokenURIs[controlTokenIndex], minValues[controlTokenIndex], maxValues[controlTokenIndex], startValues[controlTokenIndex],{from:ARTIST_C}).then(function(tx) {controlTokenIndex++;console.log("ControlToken = " + controlTokenIds[controlTokenIndex]);return artworkInstance.setupControlToken(expectedArtworkTokenId, controlTokenIds[controlTokenIndex], controlTokenURIs[controlTokenIndex], minValues[controlTokenIndex], maxValues[controlTokenIndex], startValues[controlTokenIndex],{from:ARTIST_C}).then(function(tx) {controlTokenIndex++;console.log("ControlToken = " + controlTokenIds[controlTokenIndex]);return artworkInstance.setupControlToken(expectedArtworkTokenId, controlTokenIds[controlTokenIndex], controlTokenURIs[controlTokenIndex], minValues[controlTokenIndex], maxValues[controlTokenIndex], startValues[controlTokenIndex],{from:ARTIST_C}).then(function(tx) {controlTokenIndex++;})})})})})
					})
				})
			});			
		});
